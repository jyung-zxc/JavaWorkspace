...
public class AirPolution {
	public static void main(String[] args) {
		 ...
		 *   
		 *   - API 서버
		 *   	- URL로 데이터에 대한 요청을 보내면 응답데이터를 반환해주는 서버로, 어플리케이션들 중
		 *     	  데이터나 서비스를 제공하는 역할을 API서버라고 부른다(카카오 API서버, 네이버 API서버 등)
		 * 		
		 * 		- A 어플리케이션이 B 어플리케이션에 필요한 데이터가 있는 경우,
		 *  	  B 어플리케이션의 API서버를 통해 요청(request)를 보내고, 
		 *        B API서버는 요청에대한 응답(response)을 돌려준다.
		 *        
		 *      - 단, API서버 요청시에는 해당 API서버가 정한 규칙(Interface)에 맞춰 요청을 보내야 한다.
		 *      
		 * 2. API 서버 요청 방식(Http Method)
		 *   - API서버에서 지정한 규칙에 따라야 하며, 전톡적인 방식의 API요청방법은 2가지가 있다.
		 *    1) Get방식
		 ...
		 *      - Ex) https://khedu.co.kr/data (쿼리스트림은 url에 노출되지 않는다)
		 *    		
