...
public class A_StringPool {
	/*
	 * String은 불변클래스 String은 기존에 담겨있던 값이 수정되는 순간 원래 담겨있던 공간에서 수정되지 않고, 새로운 저장공간을 할당한
	 * 후, 거기에 값을 추가한다.
	 */
...
//		str4 += "world";
		...
		/*
		 *  자바에서는 문자열 리터럴 사용시 상수풀에 값을 저장한ㄷ.
		 *  
		 *  문자열 상수풀 동적방식(jvm)
		 ...
		 *  3. 만약 이미 상수풀에 그 값이 저장되어 있다면, 저장되어있는 주소값을 반환한ㄷ.(얕은복사)
		 */
		 ...
		 *    데이터가 있는지 없는지 검사를 실행하고, 이미 값이 있다면 해당 주소값을 변수에 할당하거나
		 *    데이터가 없었다면 상수풀에 새롭게 값을 등록하고 그 주소값을 변환한다. 
		 */
	 ...
	 * String이 메모리영역에서 차지하는 비중이 크기 떄문에 메모리 부족 이슈가 종종 발생.
