...
public class run {
	public static void main(String[] args) {
		/*
		 * '=' (대입연산자)를 기준으로 왼쪽과 오른쪽의 자료형은 항상 같아야 한다. 1. 부모타입 참조변수로 부모타입 객체를 다루는 경우
		 */
		...
		System.out.println("2. 자식타입 참조변수로 자식타입 객체를 다루기");
		...
		System.out.println("3. 부모타입 참조변수로 자식타입 객체를 다루는 방법.");
		...
		/*
		 * 대입연산자 기준 양쪽의 자료형이 다름에도 불구하고 오류가 나지 않는다. 이유는, 자식객체가 부모자료형으로 '자동형변환'되고 있기 때문.
		 * 상속구조의 경우 클래스간의 형변환이 가능
		 * 
		 * 클래스간 형변환 1. UpCasting - 자식타입에서 부모타입으로 자동 형변환 - 자동형변환이 가능하므로 형변환연산자는 생략이 가능 -
		 * ex) 자식객체.부모메서드(); 부모참조변수 = 자식객체;
		 * 
		 * 2. DownCasting - 부모타입에서 자식타입으로 형변환 되는 경우 - 생략이 불가능(강제 형변환 필요) - ex)
		 * ((자식)부모).자식메서드();
		 * 
		 * 다형성을 사용하는 이유 - 부모타입으로부터 파생된 여러가지 타입의 자식 객체들을 "하나의 부모클래스로" 다룰 수 있기 때문.
		 */
		...
		for (int i = 0; i < arr.length; i++) {
			// 각 인덱스 별로 실제 참조하고있는 자식클래스로 적절히 다운캐스팅 후, 메서드를 호출하려면?
			// 1. instanceof 연산자 활용.
