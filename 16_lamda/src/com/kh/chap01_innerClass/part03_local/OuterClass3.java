...
public class OuterClass3 {
	 ...
	 *   - 메서드 안에서만 사용이 가능하며, 메서드가 호출될때 생성된다.
	 ...
	 *    - 메서드 프레임 내부에는 지역변수를 위한 저장곤간이 할당된다.
	 *    - 메서드 종료시에는 메서드프레임이 제거되면서 지역변수의 저장공간도 함께 소멸된다.
	 *   
	 *   2) static 키워드가 붙은 변수는 static메모리 영역에 저장공간이 할당된다.
	...
	public Runnable getRunnalbe(int num2) {

		int local = 1; // 지역변수
//		local = 5;
		
		//지역변수는 지역내부클래스에서 사용되는 경우, final 에약어가 붙은 것처럼 발급한다.
		// why? static 메모리에서 지역변수의 저장공간이 소멸된 후 , 참조를 유지하기위해
