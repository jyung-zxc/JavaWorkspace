...
public class StreamMethods {
	public static void main(String[] args) {
		 ...
		 *  5) Reduction 계열 : reduce메서드를 통해 요소 내부의 값을 
		 *     다양한 방법으로 연산 후 결과값으로 반환하는 메서드
		 *     Calculating계열에서 제공하지 않는 연산기능을 수행할 때 사용한다,   
		 */
		
		//중간처리 메서드(distinct, filter, forEach)
		List<Integer> list = Arrays.asList(5,1,2,3,4,2,1,3,1,2,7,6,4,3);
		...
		String[] names = {"강감찬","강원래","홍길동","강형욱","조난경"};
		...
		// 2) map : 현재 스트림요소를 다른 요소로 바꾸는 메소드
		...
		List<Integer> list4 = Arrays.asList(0,1,1,1,2,2,3,3,4,5,6,4,5);
		Set<Integer> set = list4.stream()
		...
		Member maxAgePerson =
				...
				// 초기값을 생략하는 경우 반환형이 Optional 이 된다
		...
		// 7-2) noneMatch : 스트림의 요소 모두가 false면 true를 반환
		bool = Arrays.asList("홍길동","123","가나다")
				.stream()
				.noneMatch(s -> s.length() > 4);
				
		// 7-3) allMatch : 스트림의 요소 모두가 false면 true 반환
		bool = Arrays.asList("홍길동","123","가나다")
				.stream()
				.allMatch(s -> s.length() <= 3);
				
		
		// 7-4) findFirst : 스트림 요소중 조건을 만족하는 첫번째 요소를 찾은 후 반환
		String str = 
				Arrays.asList("홍길동","2111","11111","1가나다")
				.stream()
		...
		// 7-5) findAny : 스트림 요소가 하나라도 존재한다면 해당 요소를 즉시 반환
		str = Arrays.asList("홍길동2","123","111111")
				.stream()
				.filter(s -> s.length()<=4)
				.findAny()
				.get();
