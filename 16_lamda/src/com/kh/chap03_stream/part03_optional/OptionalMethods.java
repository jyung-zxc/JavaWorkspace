...
//	 *  - Optional을 객체로 사용할 때 Optional 내부에 데이터가 있는지 없는지 
//	 *    항상 확인하는 절차를 강제 함으로써 에러를 방지한다.
//	 *  - Optional객체를 변수에 저장하여 관리하는 경우, 
//	 *    변수명에 maybe, opt와 같은 Optional 타입을 암묵적으로 제시한다. 
//	 * 
//	 */
...
//		 *  2. Optional.of(obj) : 결코 null이 아닌 객체를 담은 Optional 객체 생성
//		 *  3. Optional.ofNullable(obj) : null일수도 있는 Optional 객체 생성 
//		 *  
//		 *  Optional 객체 내부 메서드들
//		 *   - get() : Optional내부의 값을 반환하는 메서드.(값이 없다면 NoSuchElement예외발생)
//		 *   - getAs자료형() : 해당 자료형의 값을 반환하는 메서드
//		 *   - orElse(T) : get()으로 꺼낼 값이 존재하지 않을 경우 반환할 값.
//		 *   - orElseGet(Supplier) : get()으로 꺼낼 값이 존재하지 않을 경우 
//		 *     람다식을 통해 구현한 값을 반환
//		 *   - orElseThrow(Supplier) : get()으로 꺼낼 값이 존재하지 않을 때
//		 *     람다식을 통해 구현한 에러객체를 호출
//		 */
...
//				.orElse(0);
	...
	public void test2() {
		...
		member.setEmail("honggil@naver.com");
...
//		// optional없이 최대한 안전(NPE)하게 email 추출하기
//		if(order != null) {
