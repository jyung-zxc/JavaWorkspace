...
public class ListRun {
	 ...
	 *   
	 *  - 컬렉션의 장점
	 *   1. 여러자료형의 데이터를 저장할 수 있다(제네릭 설정을 통해 한가지 유형의 데이터만 저장도 가능)
	 *   2. 중간에 값을 추가하거나, 삭제하는등 배열내부의 값들을 조작하기위한 편리한 메서드가 정의되어 있다.
...
//		List list = new ArrayList(); //크기 10배열 생성
		...
		list.add(new Music("멍멍","김현정"));
		System.out.println(list); 
		
		// 2. add(int index, E e)
		// - 리스트에 전달되는 index값 위치에 e를 추가하는 메서드
		list.add(0,new Music("바아의왕자","박부장"));
		...
		// 5. size()
		//  - 리스트에 초기화된 데이터 수를 반환하는 메소드
		System.out.println("리스트에 담긴 데이터 수 : " + list.size());
		
		// 6. get(int index) : E
		//  - 리스트에서 index위치의 값을 반환해주는 메소드
		Music m1 = list.get(0);
		System.out.println(m1);
		
		// 만약 제네릭 설정을 안했다면?
		Music m2 = (Music) list.get(1); // 강제형변환 필요 (다운캐스팅)
		...
		while(iter.hasNext()) {
			...
			//iter.remove(); // List내부의 값을 삭제시키는 메서드.
		...
		//  - index1 부터 index2 까지 데이터를 추출한 새로운 sublist를 반환하는 메서드
...
//		list.clear();
		 ...
		 * 	- VO클래스에 직접 상속시키면서 상속
		 * - 해당 VO클래스의 "기본정렬조건"ㅇ,러 시영
		 * 
		 *  2. comparator 인터페이스 상속
		 *   - 기본정렬조건 외에 추가 정렬조건을 만들고자 할 때 사용
		 *   - vo클래스 이외 별도 클래스에 Comparator을 상속시켜서 구현한다.
		 *   - 여러개의 정렬조건을 만들 수 있다.
		 */
