...
public class Run {
	public static void main(String[] args) {
		 ...
		 *  - 컬렉션에 데이터를 추가시 항상 Object 객체로 저장해야 한다.
		 ...
		 *  - 클래스의 메서드의 "반환형", "대개변수", 필드의 "자료형들을 
		 *    미리 확정짓지 않고 파라미터화 하여 나중에 지정할 수 있게 만드는 문법.
		 *  - 즉, 제네릭을 사용하면 다양한 자료형(타입)들을 저장할 수 있기 때문에 타입변수 라고도 불린다.
		 *  - 제네릭을 사용하면 런타임이 아닌 컴파일 시점에서 에러를 캐치할 수 있으므로 타입안전성을 확보할 수 있다.
		 *  
		 */
		Generic<Integer, String> g1 = new Generic<>();
		g1.setT(1);
		g1.setG("g");
		Generic.printTest("rrrr"); // 제네릭 미지정시 Object
		Generic.printTest(1919);
//		Generic<String>printTest(1111);
		
		// 대입될 데이터의 자료형을 미리 지정하지 않고, 컴파일 시점에서 내 맘대로 지정하며 
		// 사용하므로 재사용성이 크게 증가하였다.
		
		/*
		 * 3. 제네릭의 extends 문법
		 *  - 제네릭에서의 extends는 클래스간의 상속 개념이 아닌, 제네릭으로 지정한 자료형의 범위를 "제한"시키는데 사용한다.
		 *  - <T extends Parent> : T(타입변수)에는 Parent 클래스나 Parent 클래스를 상속받은 자료형만 대입 가능
		 *  - <T extends Interface> : InterFace를 구현한 클래스만 대입가능.
		 */
		
		GenericExtends<Parent> parent = new GenericExtends<>();
		GenericExtends<Child1> child1 = new GenericExtends<>();
	// Parent및 Parent자식이 아니기떄문에 설정 오류
//		GenericExtends<String> str = new GenericExtends<String>();
		...
		/*
		 * 4. 와일드카드(?)
		 *  - Generic에 들어가야할 자료형읆 모르는 경우 사용
		 *  - 와일드카드(?)의 의미는 어떤 자료형이 들어올지 모른다(unknown)의 의미로 사용하며,
		 *    모든자료형(any)이 들어올 수 있다는 의미는 아니다.
		 *  - ?는 extends와 supar예약어로 자료형의 범위를 제한할 수 있다.
		 *  - ? extends T : 상한제한 => 와일드카드의 범위를 T 또는 T의 자식들로 제한 가능
		 *  - ? super T : 하한제한 => 화일드카드읨 범위를 T 또는 T의 "조상"들로 제한
		 *  
		 */
...
//		unknown.setType(new Parent);
		...
		// ?의 제대로된 사용을 위해선 extends와 super를 사용하여 ?로 범위를 제한해야 한다.
...
//		unknown2.setType(new Child1());
		// ?의 범위에 Child1도 포함되어 있으므로 대입불가능한 상황이 생긴다.
//		unknown2.setType(new Child2());
		
		// unknown2가 Child1 이든 Child2든, Parent든, 다형성에 의해 대입 가능
		// 즉, extends를 통한 상한제한은 데이터를 "가져올때"자주 사용한다.
		Parent p = unknown2.getType();
		
		// 2) super를 통한 와일드카드 범위 하한 제한
		// ? uper Parent : 와일드카드의 범위는 최하 Parent부터 최상 Object
		...
		// 값을 "대입"하고자 할때는 super을 통한 하한제한을 사용하는 것이 저렵하다.
		
		Parent p3 = unknown3.getType(); // ?는 Object일 수 있음
		// 단, 클래스레벨 제네릭 설정시 T.extends Parent로 설정했기 떄문에 범위는 Parent - Parent의 자식클래스로 한정
		
		// 따랏 ㅓ타입변수에 extends를 통한 타입제한이 없는 경우,
		// 데이터는 항상 Object형태로만 추출이 가능
