...
public class Run {
	...
	public void test1() {
		 ...
		 *   - 지정된 사긴이 지나면, 다시 실행대기(Runnable)상태로 변경된다.
		 *   
		 *  2) wait(ms)
		 *   - 동기화된 블럭안에서만 사용되며, 지정된 시간이 지나거나 다른 스레드가 호출하기 전까지 대기시키는 메서드.
		 *   
		 *  3) join(ms)
		 *   - 지정된 시간 동안 다른 스레드를 실행시키고 현재 스레드는 대기시키는 메서드,
		 *     지정된 시간이 지나거나 다른 스레드의 작업이 종료되면 다시 Runnable상태로 변경
		 *     
		 *  4) IO Block
		 *   - 입출력메서드에 의해 일시정지된 상태로, 입출력 완료시 다시 Runnalbe상태로 변경
		 *  
		 */
	...
	public void test2() {
		/*
		 * 스레드 컨트롤 메서드
		 *  - 일시정지(TimeWating)상태인 스레드를 다시 Runnable상태로 변경하느 네멋드
		 *  1) notify() / notifyAll()
		 *   - 동기화된 블럭안에서만 사용하며, 대기중인 쓰레드를 채우는 (Runnalbe)메서드
		 *  2) interrupt()
		 *   - sleep나, join에 의해 일시정지된 메서드를 채우는 역할의 메서드로,
