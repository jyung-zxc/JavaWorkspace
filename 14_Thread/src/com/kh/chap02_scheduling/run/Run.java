...
public class Run {
	public static void main(String[] args) {
		 ...
		 *  - 만약, 스레드간에 우선순위가 부여된 경우, 
		 *    우선순위가 높은 스레드가 더 많은 작업시간을 할당받는다.(우선순위 기반 스케쥴링)
		...
		Thread th1 = new Thread(() -> {
			// 현재 스레드의 이름
			System.out.println(Thread.currentThread().getName());// 현재 실행중인 스레드의 이름
			for(int i = 0; i<20; i++) {
		...
		th1.setDaemon(true);
		th2.setDaemon(true);
		th3.setDaemon(true); // 모든스레드를 데몬 스레드로 설정해야, 설정완료
